...
class Diff{
  ...
  /* DD custom output
   */
  public static function toHTML_DD($diff){
  	...
  	foreach ($diff as $line){
  		...
  		switch ($line[1]){
  			case self::UNMODIFIED : $element = ''; $end_element = ''; break;
  			case self::DELETED    : $element = '<span class="bg-danger text-white">'; $end_element = '</span>'; break;
  			case self::INSERTED   : $element = '<span class="bg-success">';  $end_element = '</span>'; break;
  		}
  		$html .=  $element . htmlspecialchars($line[0]) . $end_element;

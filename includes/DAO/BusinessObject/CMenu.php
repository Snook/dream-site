...
class CMenu extends DAO_Menu
{
	...
	function whereAdd_Menus($is_active = true, $number_of_past_months = 0): void
	{
		if ($is_active)
		{
			$this->is_active = 1;
	...
	function isEnabled_ShippingDiscount($DAO_store): bool
	{
		$optOutStores = array(
			// Sacramento
			108,
	...
	function isEnabled_Markup(): bool
	{
		if ($this->id >= 265)
		{
			return false;
	...
	function isEnabled_Markup_Sides(): bool
	{
		if ($this->id >= 272)
		{
			return false;
	...
	function isEnabled_Add_Sides_and_EFL(): bool
	{
		if ($this->id >= 273)
		{
			return false;
	...
	function isEnabled_MarkDown(): bool
	{
		// Currently same rules as isEnabled_Markup()
		return $this->isEnabled_Markup();
	}

	function isEnabled_MarkupRoundUp(): bool
	{
		// Currently same rules as isEnabled_Markup()
		return $this->isEnabled_Markup();
	}

	function isEnabled_Bundle_Fundraiser(): bool
	{
		if ($this->id >= 265)
		{
			return false;
	...
	function isEnabled_Assembly_Fee(): bool
	{
		if (is_null($this->id))
		{
			return true;
	...
	function isEnabled_OrderManager_Fundraiser(): bool
	{
		// Currently same rules as isEnabled_Bundle_Fundraiser()
		return $this->isEnabled_Bundle_Fundraiser();
	}

	function isEnabled_Starter_Pack_Bundle(): bool
	{
		if ($this->id >= 269)
		{
			return false;
	...
	function isEnabled_Starter_Pack($DAO_store): bool
	{
	 ...
	 * @throws Exception
	 */
	static function storeSpecificMenuExists($menu_id, $store_id): bool
	{
	...
	function areSessionsOrdersEditable($storeObj = false, $timezone_id = false): bool
	{
	...
	// Use in conjunction with getActiveMenuArray - you must pass in the last active menu to be sure that menus left with display_as_coming_soon set to true are not inadvertently returned
	 ...
	 * @throws Exception
	 */
	function isTimeStampLegalForMenu($inTimeStamp): bool
	{

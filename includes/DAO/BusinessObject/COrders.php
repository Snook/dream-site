...
class COrders extends DAO_Orders
{
	 ...
	 *  calculations more efficient. These totals are *not* used in calculating any other order fields
	 */
...
*/
	...
	static public function sendConfirmationEmail($user, $order, $includeGiftCardMessage = false, $delayedTransaction = false, $shouldSendGiftMessage = true)
	{
		...
		{
			$contentsText = CMail::mailMerge('order_delivered.txt.php', $orderInfo);
			$contentsHtml = CMail::mailMerge('order_delivered.html.php', $orderInfo);
	...
	public static function buildOrderItemsArray($Order, $promo = null, $freeMeal = null, $flatList = false, $orderBy = 'FeaturedFirst')
	{
		...
		while ($DAO_menu_item->fetch())
		{
			...
			if (!empty($DAO_menu_item->DAO_order_item->parent_menu_item_id))
			{
				...
				{
					$parentItems[$DAO_menu_item->DAO_order_item->parent_menu_item_id]['display_title_pre_assembled'] .= "<br /><span style='font-size:smaller;'>&nbsp;&nbsp;&nbsp;" . $DAO_menu_item->bundle_item_count . " " . $DAO_menu_item->menu_item_name . "</span>";

...
try
{
	...
	$storeCredits->query("select sub_query.*
		from (select sc.*, cr.origination_type_code, CONCAT(u.firstname, ' ', u.lastname) as fullname, u.primary_email, s.email_address as store_email, s.store_name, if(sc.date_original_credit, sc.date_original_credit, sc.timestamp_created) as basedate 
			from store_credit sc 
				join user u on u.id = sc.user_id 
				join store s on s.id = sc.store_id 
				left join customer_referral cr on cr.store_credit_id = sc.id where sc.is_redeemed = 0 and sc.is_expired = 0 and sc.is_deleted = 0 and sc.was_sent_60_day_warning = 0 and credit_type = 2) as sub_query 

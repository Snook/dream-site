...
try {
	$path = "C:\\Users\\Carl.Samuelson\\Zend\workspaces\\DefaultWorkspace12\\DreamSite\\gc_unloads.csv";
	...
	while (!feof($fp))
	{
		...
		$acct_check = DAO_CFactory::create('gift_card_transaction');
		...
		$acct_check->query("update gift_card_transaction set clear_card_number = '{$valArr[1]}' where id = {$valArr[0]};");

		$acct_set = DAO_CFactory::create('gift_card_transaction');

		$message = CCrypto::encode(base64_encode($valArr[1]));
    	$acct_set->query("Call store_gcan_trans('$message', {$valArr[0]})");
    	$totalCount++;
	}

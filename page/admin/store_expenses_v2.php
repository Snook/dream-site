...
class page_admin_store_expenses_v2 extends CPageAdminOnly
{
	...
	function FindDates($store_id, $Day, $Month, $Year, $Interval = '1 MONTH')
	{
		...
		{
			$thisDate = date("n/j/Y", strtotime($store_expenses->entry_date));
			$thisItem = array(
				'time' => $current_date_sql,
				'type' => $store_expenses->expense_type,
				'amount' => $store_expenses->total_cost,
				'units' => $store_expenses->units,
				'notes' => $store_expenses->notes,
				'session_id' => $store_expenses->session_id
			);
			$rows[$thisDate][$store_expenses->expense_type] = $thisItem;
		}

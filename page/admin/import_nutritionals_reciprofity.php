...
class page_admin_import_nutritionals_reciprofity extends CPageAdminOnly
{
	...
	function importNuts()
	{
		...
		if (!empty($_POST['menu']) && !empty($_FILES['base_menu_import']) && $_FILES['base_menu_import']['error'] == UPLOAD_ERR_OK && is_uploaded_file($_FILES['base_menu_import']['tmp_name']))
		{
			...
			try
			{
				$uberObject = DAO_CFActory::create('menu_item', true);
				...
				{
					$tpl->setStatusMsg('<p>Nutrition imported: <a class="btn btn-primary btn-sm" href="/nutritionals?store=166&amp;menu=' . CGPC::do_clean($_POST['menu'], TYPE_INT) . '&amp;show_entree=1&amp;show_efl=1&amp;show_ft=1&amp;filter_zero_inventory=1" target="_blank">Review</a></p><p>Next Step: <a class="btn btn-primary btn-sm" href="/backoffice/import_sidesmap_reciprofity">Import Sides Map</a></p>');
				}
		...
		try
		{
			...
			foreach ($exCompsArr as $thisComp)
			{
				if (empty($thisComp->hasBeenUpdated))
				{
					...
					$thisNutData = DAO_CFactory::create('nutrition_data', true);
			...
			$existingNullComps = DAO_CFactory::create('recipe_component', true);
			...
			foreach ($componentArray as $thisField)
			{
				...
				$thisNutElement = DAO_CFactory::create('nutrition_element', true);
				...
				foreach ($theseNuts as $thisNut)
				{
					...
					// default recipe
					$thisNutDataNull = DAO_CFactory::create('nutrition_data', true);
		...
		try
		{
			...
			foreach ($componentArray as $thisField)
			{
				...
				foreach ($theseNuts as $thisNut)
				{
					...
					// default recipe
					$thisDefaultNutData = DAO_CFactory::create('nutrition_data', true);

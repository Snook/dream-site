...
class page_admin_reports_financial_performance extends CPageAdminOnly
{
	...
	static     $rankingFields = array(array('name' => 'cost_of_goods_and_services', 'convert' => true), 
                               array('name' => 'employee_wages', 'convert' => true),
                               array('name' => 'employee_hours', 'convert' => false),
                                array('name' => 'manager_hours', 'convert' => false),
                                array('name' => 'bank_card_merchant_fees', 'convert' => true),
                                array('name' => 'kitchen_and_office_supplies', 'convert' => true),
                                array('name' => 'total_marketing_and_advertising_expense', 'convert' => true),
                               array('name' =>  'rent_expense', 'convert' => true),
                                array('name' => 'repairs_and_maintenance', 'convert' => true),
                                array('name' => 'utilities', 'convert' => true),
                               array('name' =>  'other_expenses', 'convert' => true),
                               array('name' =>  'net_income', 'convert' => false));
	
	static     $convertFields = array('cost_of_goods_and_services', 'owner_salaries', 'manager_salaries', 'employee_wages','bank_card_merchant_fees',
	                           'kitchen_and_office_supplies', 'national_marketing_fee', 'salesforce_fee', 'royalty_fee', 'total_marketing_and_advertising_expense','rent_expense',
	                          'repairs_and_maintenance','utilities','other_expenses', 'monthly_debt_service', 'payroll_taxes');
	    
	static		$hoursFields = array( 'employee_hours','manager_hours', 'owner_hours');
	...
	function runFranchiseManager()
	{
	    ...
	    {
	        CApp::bounce('main.php?page=admin_access_error&pagename=Financial Performance Report');
	    }
	...
	function runFinPerfReport()
	{
		...
		$non_expense_arr = array("gross_sales", "mark_up", "adjustments_and_discounts", "adjusted_gross_revenue", "net_income", "discounts_total", "agr_total");
		
		$hours_field = array("employee_hours", "owner_hours", "manager_hours");
		...
		if ( $Form->value('report_submit') || $export)
		{
		    ...
		    if ($Form->value('date_type') == 'single_month')
		    {
		        ...
		        $rows = array("gross_sales" => array("Gross Sales", $storeInfo['gross_sales']),
		                          "mark_up" => array("+ Mark Up", $storeInfo['mark_up']),
		                          "discounts_total" => array("- Adjustments & Discounts", $storeInfo['adjustments_and_discounts']),
		                          "agr_total" => array("Adjusted Gross Revenue", $storeInfo['adjusted_gross_revenue']));
		        ...
		        {
		        	$rows['national_marketing_fee'] = array("National Marketing Fee", $storeInfo['marketing_total']);
		        }
		        else
		        {
		        ...
		        {
		        	$rows['royalty_fee'] = array("Royalty Fee", $storeInfo['royalty']);
		        }
		        else
		        {
		        ...
		        $columnDescs[$colSecondChar.$col] = array('align' => 'left', 'width' => 36, 'type' => 'text');
		        incrementColumn($thirdSecondChar, $colSecondChar, $col);
		        $columnDescs[$colSecondChar.$col] = array('align' => 'right', 'width' => 12, 'type' => 'currency_no_cents');
		        incrementColumn($thirdSecondChar, $colSecondChar, $col);
		        ...
		        if (isset($_POST['show_comparisons']))
		        {
		            ...
		            $headerNames = array("Financial Performance Report", $monthLabeling, "Nat'l Avg", "Nat'l Rank", "Region Avg", "Region Rank", "Revenue Tier Avg", "Revenue Tier Rank");
		            
		            $columnDescs[$colSecondChar.$col] = array('align' => 'right', 'width' => 12, 'type' => 'currency_no_cents');
		            incrementColumn($thirdSecondChar, $colSecondChar, $col);
		            $columnDescs[$colSecondChar.$col] = array('align' => 'center', 'width' => 12, 'type' => 'text');
		            incrementColumn($thirdSecondChar, $colSecondChar, $col);
		            $columnDescs[$colSecondChar.$col] = array('align' => 'right', 'width' => 12, 'type' => 'currency_no_cents');
		            incrementColumn($thirdSecondChar, $colSecondChar, $col);
		            $columnDescs[$colSecondChar.$col] = array('align' => 'center', 'width' => 12, 'type' => 'text');
		            ...
		            if (CUser::getCurrentUser()->user_type == CUser::SITE_ADMIN || CUser::getCurrentUser()->user_type == CUser::HOME_OFFICE_MANAGER)
		            {
		                ...
		                $headerNames = array_merge($headerNames, array("Sysco Opco Avg", "Sysco Opco Rank"));
		             }
		            
		        }
		        else
		        {
		            $headerNames = array("Financial Performance Report", $monthLabeling);
		        }
		        
		        $numRows = count($rows);
		        $callbacks = array('row_callback' => 'finPerfReportRowsCallback', 'cell_callback' => 'finPerfReportComparisonCellCallback');
		    ...
		    else // month range
		    {
		        ...
		        $labels = array ("gross_sales" => "Gross Sales",
		            "mark_up" => "+ Mark Up",
		        ...
		        if ($rows)
		        {
    		        ...
    		        $columnDescs[$colSecondChar.$col] = array('align' => 'left', 'width' => 50, 'type' => 'text');
    		        incrementColumn($thirdSecondChar, $colSecondChar, $col);
    		         
    		        $numDataCols = count($headerNames) - 1;
    		        for($x = 0; $x < $numDataCols; $x++)
    		        {
    		            $columnDescs[$colSecondChar.$col] = array('align' => 'right', 'width' => 15, 'type' => 'currency_no_cents', 'decor' => 'bottom_border');
    		            incrementColumn($thirdSecondChar, $colSecondChar, $col);
    		        }
    		        
    		        $callbacks = array('row_callback' => 'finPerfReportRowsCallback', "cell_callback" => 'finPerfReportCellCallback');
	...
	function retrieveStoreData($store_id, $Form, $selectMonth, $labels, $rows)
	{
        ...
        if($queryObj->fetch())
        {
            ...
            foreach($dieser as $dbName => $thisField)
            {
                ...
                $rows[$dbName] = array ( $accountName, $thisField);
            }
        }
    ...
    function retrieveComparisonData($store_id, &$rows, $selectMonth, $curMonth, $curYear, $store_trade_area, $store_class, $opco_id)
    {
        ...
        $nonPLArr = array("gross_sales", "mark_up", "discounts_total", "agr_total");
        $percentageBasedArr = array('cost_of_goods_and_services', 'employee_wages', 'bank_card_merchant_fees', 'kitchen_and_office_supplies', 'utilities', 'other_expenses', 'total_marketing_and_advertising_expense', 'rent_expense', 'repairs_and_maintenance');
        $sensitiveFieldArr = array("manager_salaries", "owner_salaries", "monthly_debt_service", "owner_hours", "payroll_taxes");
    ...
    function getPandLByMonthAllStores($curMonth, $curYear, &$data, $store_id, $store_trade_area = false, $store_class = false, $opco_id = false, $logData = false)
    {
        ...
        while ($p_and_l->fetch())
        {
        	...
        	if (!isset($data[$p_and_l->store_id])) { $data[$p_and_l->store_id] = array(); }
                ...
                if (isset($data[$p_and_l->store_id][$thisFieldName]))
                {
                    if (!is_null($data[$p_and_l->store_id][$thisFieldName]) && $data[$p_and_l->store_id][$thisFieldName] !== "")
                    {
                        if (isset($averages[$thisFieldName]))
                         ...
                         {
                             $averages[$thisFieldName] = array('store_count' => 1, 'value' => $data[$p_and_l->store_id][$thisFieldName]);
                         }
                    }
                }
        ...
        return array($rankings, $averages);
    }
    ...
    function getOrderInfoByMonthRange ($startMonth,  $endMonth, &$rows, $store_id, &$headerRow)
    {
        ...
        while ($session->fetch())
        {
            ...
            if (empty($tmpRows[$monthLabel]['fundraising_total'])) $tmpRows[$monthLabel]['fundraising_total'] = 0;
			if (empty($tmpRows[$monthLabel]['ltd_item_donation_total'])) $tmpRows[$monthLabel]['ltd_item_donation_total'] = 0;
			if (empty($tmpRows[$monthLabel]['subtotal_delivery_fee'])) $tmpRows[$monthLabel]['subtotal_delivery_fee'] = 0;
			if (empty($tmpRows[$monthLabel]['subtotal_bag_fee'])) $tmpRows[$monthLabel]['subtotal_bag_fee'] = 0;
            ...
            $tmpRows[$monthLabel]['adjustments_and_discounts'] =  $tmpRows[$monthLabel]['total_discounts'] + $tmpRows[$monthLabel]['discounts'] +
                                                                $tmpRows[$monthLabel]['fundraising_total'] + $tmpRows[$monthLabel]['ltd_item_donation_total'] + $tmpRows[$monthLabel]['subtotal_delivery_fee'];
            ...
            $rows[$monthLabel] = array( 'gross_sales' => $gross_sales, 'mark_up' => $tmpRows[$monthLabel]['mark_up'],
                'adjustments_and_discounts' => $tmpRows[$monthLabel]['adjustments_and_discounts'], 'adjusted_gross_revenue' => $tmpRows[$monthLabel]['adjusted_gross_revenue'], 'national_marketing_fee' => $marketingFee, 'salesforce_fee' => $salesForceFee, 'royalty_fee' => $royaltyFee);
    ...
    function getOrderInfoByMonthAllStores ($Day, $Month, $Year,  $interval, &$rows, $store_id, $store_trade_area = false, $store_class = false, $opco_id = false)
    {
        ...
        while ($session->fetch())
        {
            ...
            if (empty($rows[$session->store_id]['fundraising_total'])) $rows[$session->store_id]['fundraising_total'] = 0;
			if (empty($rows[$session->store_id]['ltd_menu_item_value'])) $rows[$session->store_id]['ltd_menu_item_value'] = 0;
			if (empty($rows[$session->store_id]['subtotal_delivery_fee'])) $rows[$session->store_id]['subtotal_delivery_fee'] = 0;
			if (empty($rows[$session->store_id]['subtotal_bag_fee'])) $rows[$session->store_id]['subtotal_bag_fee'] = 0;
            ...
            CDreamReport::calculateFees (	
            	$rows[$session->store_id], 
            	$stid,
            	$haspermanceoverride,
            	$expenseData, 
            	$giftCertValues , 
            	...
            	$marketingFee, 
            	$royaltyFee, 
            	$thisStoreInfo->grand_opening_date, 
            	$Month,
            	$Year);
            ...
            $rows[$session->store_id]['adjustments_and_discounts'] =  $rows[$session->store_id]['total_discounts'] + $rows[$session->store_id]['discounts'] +
            					 $rows[$session->store_id]['adjustments'] + $rows[$session->store_id]['fundraising_total'] + $rows[$session->store_id]['ltd_menu_item_value']
				+ $rows[$session->store_id]['subtotal_delivery_fee'];
        ...
        return array('gross_sales_avg' => $gross_sales_total / $countStores,
...
function sort_by_cost_of_goods_and_services($a, $b)
{
    ...
    if (empty($a[$fieldName])) $a[$fieldName] = 0;
    if (empty($b[$fieldName])) $b[$fieldName] = 0;

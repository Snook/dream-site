...
class page_admin_menu_editor extends CPageAdminOnly
{
	...
	function runMenuEditor()
	{
		...
		$array_DAO_menu->whereAdd("menu.global_menu_start_date >= DATE_FORMAT(NOW(),'%Y-%m-%d') OR ( menu.global_menu_start_date <= DATE_FORMAT(NOW(),'%Y-%m-%d') AND menu.global_menu_end_date >= DATE_FORMAT(NOW() - INTERVAL 1 MONTH,'%Y-%m-%d') )");

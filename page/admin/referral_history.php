...
class page_admin_referral_history extends CPageAdminOnly {
...
//CES: ACCESS_CHANGE
	 ...
	 function runSiteAdmin() {
			...
			if (!empty($totalsArray))
			{
				...
				$StoreCredit->query("select sc.store_id, st.store_name, SUM(sc.amount)as amount_total from store_credit sc " .
									" join store st on st.id = sc.store_id " .
									" where sc.user_id = $id and sc.is_deleted = 0 and sc.is_expired = 0 and sc.is_redeemed = 1 " .
			...
			if ($storeSupportsPlatePoints)
			{
				...
				$creditsObj->query("select pc.id, pc.timestamp_created, pc.credit_state, pc.dollar_value, pc.expiration_date, GROUP_CONCAT(puh.event_type) as events from points_credits pc
									...
									group by pc.id
									order by pc.credit_state, pc.id desc");

				// DD EXP DATE DISPLAY 6
				while($creditsObj->fetch())
				{
					$rows[$creditsObj->id] = array("timestamp" => $creditsObj->timestamp_created, 'expires' => $creditsObj->expiration_date, 'amount' => $creditsObj->dollar_value, 'state' => $creditsObj->credit_state, 'events' => $creditsObj->events);

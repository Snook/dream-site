...
class page_admin_reports_goal_management_v2 extends CPageAdminOnly
{
	...
	function joinEmptySessions(&$masterArray, $nowAtStore, $menu_id, &$hasSessionToday)
	{
		...
		while($sessionFinderObj->fetch())
		{
			...
			if (!isset($masterArray[$weeknum][$dayNum][$sessionFinderObj->id]))
			{
				$masterArray[$weeknum][$dayNum][$sessionFinderObj->id] = $this->createEmptySessionArray($sessionFinderObj, $isPast, $sessionTimeTS);
	...
	function retrieveMetricsArray($month, $year, &$sessionLeads, $tpl, &$hasSessionToday, &$menu_start, &$menu_end, &$menu_id)
	{
		...
		, sum(iq.grand_total) - sum(iq.subtotal_all_taxes) as gross_revenue
		 from (
		select od.user_id, od.order_id, od.session_time, od.session_id, od.user_state, od.order_type, od2.id, od.addon_total, od2.id as in_store, o.grand_total, o.subtotal_all_taxes  from orders_digest od
		...
		join session s on s.id = iq.session_id and s.store_id = {$this->currentStore}  and s.is_deleted = 0 and (s.session_type_subtype != 'WALK_IN' || s.session_type_subtype is null)
			group by s.id 
	...
	function runFranchiseManager()
	{
 	...
 	function runSiteAdmin()
 	{
		...
		else
		{
			...
			$menuAnchorTS = strtotime($curMenuArr['menu_start']);
			$year = date("Y", $menuAnchorTS);

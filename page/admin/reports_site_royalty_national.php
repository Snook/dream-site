	...
	function runHomeOfficeManager() {
 	...
 	function runSiteAdmin() {
		...
		$Form->AddElement(array (CForm::type => CForm::Submit,
 		CForm::name => 'report_submit', CForm::value => 'Run Web Report'));

        $Form->AddElement(array (CForm::type => CForm::Submit,
            CForm::name => 'report_export', CForm::value => 'Export Report'));
        ...
        if ( $Form->value('report_export')) {
				...
				if ($row != NULL && count($row) > 0) {
					...
					for ($i = 0; $i < count($row); $i++)
					{
					    ...
					    if (empty($row[$i]['fundraiser_total'])) $row[$i]['fundraiser_total'] = 0;
					    if (empty($row[$i]['ltd_round_up_value'])) $row[$i]['ltd_round_up_value'] = 0;
                        if (empty($row[$i]['subtotal_delivery_fee'])) $row[$i]['subtotal_delivery_fee'] = 0;
						if (empty($row[$i]['subtotal_bag_fee'])) $row[$i]['subtotal_bag_fee'] = 0;
						...
						CDreamReport::calculateFees ($row[$i],$storeid,$haspermanceoverride, $storeexpenseData, $giftCertDataStore , $progDataStore, $row[$i]['fundraiser_total'], $row[$i]['ltd_menu_item_value'],
							$row[$i]['subtotal_delivery_fee'], $row[$i]['subtotal_bag_fee'], $DoorDashFees, $marketingFee, $royaltyFee, $row[$i]['grand_opening_date'], $month, $year);
		...
		if ( $Form->value('report_submit')) {
			...
			if ($row != NULL && count($row) > 1) {
				for ($i = 0; $i < count($row); $i++)
				{
				    ...
				    CDreamReport::calculateFees ($row[$i], $storeid,$haspermanceoverride, $storeexpenseData, $giftCertDataStore , $programDataStore, $row[$i]['fundraiser_total'], $row[$i]['ltd_menu_item_value'],
						$row[$i]['subtotal_delivery_fee'], $row[$i]['subtotal_bag_fee'], $DoorDashFees, $marketingFee, $royaltyFee, $row[$i]['grand_opening_date'], $month, $year);
					...
					if (isset($homeofficeids[$storeid])) {
						foreach($homeofficeids[$storeid] as $element){
							...
							$row[$i]['store_history'][] = array($element['recorded_home_office_id'],$element['recorded_grand_opening_date']) ;

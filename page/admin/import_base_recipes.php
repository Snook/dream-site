	...
	function importRecipes()
	{
		...
		if (!empty($_FILES['base_recipe_import']) && $_FILES['base_recipe_import']['error'] == UPLOAD_ERR_OK && is_uploaded_file($_FILES['base_recipe_import']['tmp_name']))
		{
			...
			try
			{
				...
				$tpl->setStatusMsg('Recipe imported ' . ($count - 1) .' items successfully.<br />Go to <a href="/backoffice/manage_manufacturer_items" class="button">Manage Labels</a> to give store access to new recipe labels.');
			}
			catch (exception $e)
			{

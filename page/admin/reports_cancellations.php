 ...
 */
 ...
 class page_admin_reports_cancellations extends CPageAdminOnly {
	...
	function runHomeOfficeStaff(){
 	...
 	function runSiteAdmin() {
		...
		if (isset ($_REQUEST["report_submit"])) {
	    	...
	    	CLog::RecordReport("Cancellation (Excel Export)", "Rows:$numRows ~ Store: $store ~ Day: $day ~ Month: $month ~ Year: $year ~ Duration: $duration" );
	    	...
	    	if ($numRows)
	    	{
				...
				{
					$labels = array_merge($labels, array("Session", "Order Type", "Order Total", "Order Link", "Time of Cancellation", "Admin Notes", "Reason", "Declined MFY option", "Declined to Reschedule"));
                    ...
                    $columnDescs[$colSecondChar.$col] = array('align' => 'left', 'type' => 'text', 'width' => '40'); //
                    incrementColumn($thirdSecondChar, $colSecondChar, $col);
                    $columnDescs[$colSecondChar.$col] = array('align' => 'center', 'type' => 'text', 'width' => '12'); //
                    incrementColumn($thirdSecondChar, $colSecondChar, $col);
                    $columnDescs[$colSecondChar.$col] = array('align' => 'center', 'type' => 'text', 'text' => '12'); //
                    incrementColumn($thirdSecondChar, $colSecondChar, $col);
				}
		...
		$tpl->assign('page_title','Cancellation Report');
	...
	function findCustomers ($store_id, $Day, $Month, $Year, $Interval, &$greatestNumberCancelled)
	{
		...
		$colcount = 22;  // MAKE SURE YOU UPDATE THIS IF YOU ADD MORE SELECT STATEMENT COLS
		$selectStr = "Select user.id, `user`.lastname,`user`.firstname, user.dream_reward_level, user.dream_rewards_version, user.dream_reward_status, 0 as last_session,  `user`.primary_email,`user`.telephone_1,
		`user`.telephone_2, Count(orders.id) AS orders_made,GROUP_CONCAT(session_start) AS sessions_attended, GROUP_CONCAT(session_type) as session_type, GROUP_CONCAT(orders.type_of_order) as order_type, GROUP_CONCAT(orders.grand_total) AS grand_total,
		GROUP_CONCAT(orders.id) as order_link, GROUP_CONCAT(orders.order_confirmation) as order_conf, GROUP_CONCAT(booking.timestamp_updated) as cancelled_times, GROUP_CONCAT(orders.order_admin_notes) as notes, booking.reason_for_cancellation, 
		if (booking.declined_MFY_option = 1, 'YES', if(isnull(booking.declined_MFY_option), '', 'NO')) as declined_MFY_option, 
		if (booking.declined_to_reschedule = 1, 'YES', if(isnull(booking.declined_to_reschedule),'', 'NO')) as  declined_to_reschedule";
		$fromStr = " From booking Inner Join orders ON booking.order_id = orders.id Inner Join session ON booking.session_id = session.id Inner Join `user` ON booking.user_id = `user`.id Inner Join address ON `user`.id = address.user_id";
		$whereStr = " Where orders.store_id = '" . $store_id . "' AND ";
		$whereStr1 =  " session_start >= '"  . $current_date_sql . "' AND " ;
		$whereStr2 =  " session_start <  DATE_ADD('" . $current_date_sql . "', INTERVAL " . $Interval . " ) AND " ;
		$whereStr3 =   ' booking.status = "CANCELLED" and session_publish_state != "SAVED" group by user.id order by user.lastname, user.firstname';
		$querystr = $selectStr .$fromStr . $whereStr. $whereStr1. $whereStr2. $whereStr3;
		...
		while ($booking->fetch()) {
		  ...
		  if (count($tempDate) > 0) {
			  ...
			  if ($countarr > 1)
			  {
			  		...
			  		$comfirmations = explode(",",$tarray['order_conf']);
			  		$cancelledTimes = explode(",",$tarray['cancelled_times']);
			  		$adminNotes = explode(",",$tarray['notes']);
                   $cancelReasons = explode(",",$tarray['reason_for_cancellation']);
                   $MFYDeclines = explode(",",$tarray['declined_MFY_option']);
                   $rescheduleDeclines = explode(",",$tarray['declined_to_reschedule']);
			    	...
			    	for ($var=0; $var < $countarr; $var++)
			    	{
			      		...
			      		$ID = $IDs[$var];
			      		$ConfNum = $comfirmations[$var];
			      		$cancelledTime = $cancelledTimes[$var];
			      		$thisNote = $adminNotes[$var];
			      		$thisReason = $cancelReasons[$var];
                        $thisMFYDecline = $MFYDeclines[$var];
                        $thisRescheduleDecline = $rescheduleDeclines[$var];


						$convertTimeStamp = PHPExcel_Shared_Date::stringToExcel($sep[$var]);
						$cancelledTime = PHPExcel_Shared_Date::stringToExcel($cancelledTime);
						...
						if ($var == 0)
						{
						    	...
						    	$tarray['order_link'] =  "=HYPERLINK(\"" . HTTPS_BASE . "main.php?page=admin_order_details&order=" . $ID ."\", \"" . $ConfNum . "\")";
                                ...
                                $tarray['declined_MFY_option'] = $thisMFYDecline;
                                $tarray['declined_to_reschedule'] = $thisRescheduleDecline;
						...
						else
						{
								...
								array_push($tarray, "=HYPERLINK(\"" . HTTPS_BASE . "main.php?page=admin_order_details&order=" . $ID ."\", \"" . $ConfNum . "\")");
                                ...
                                array_push($tarray, $thisMFYDecline);
                                array_push($tarray, $thisRescheduleDecline);
                            }
				 	}
			 	}
			 else
			 {
                ...
                else
                {
                 ...
                 $tarray['order_link'] = "=HYPERLINK(\"" . HTTPS_BASE . "main.php?page=admin_order_details&order=" . $tarray['order_link'] ."\", \"" . $tarray['order_conf'] . "\")";

...
class page_admin_main_delivered extends CPageAdminOnly
{
	...
	function runPageMain()
	{
		...
		{
			$session_info_array = CSession::getDeliveredSessionDetailArray($_REQUEST['session']);
			...
			{
				$request_date = strtotime(CGPC::do_clean($session_info_array[$_REQUEST['session']]['session_start'],TYPE_STR));
			}
		}
		...
		if ($this->show['store_selector'])
		{
			...
			if (!empty($session_info_array))
			{
				$Form->DefaultValues['store'] = CGPC::do_clean($session_info_array[$_REQUEST['session']]['store_id'],TYPE_INT);
			}
			else
			{
		...
		$sessionsArray = CSession::getMonthlySessionInfoArrayForDelivered($Store, $request_date, false, $cart_info, false, true);
		...
		{
			$tpl->assign('selected_menu_id', CGPC::do_clean($session_info_array[$_REQUEST['session']]['menu_id'],TYPE_INT));
		}
		else
		{

...
class page_admin_emergency_main extends CPageAdminOnly
{
	...
	function runManufacturerStaff()
	{
		...
		CApp::bounce('main.php?page=admin_reports_manufacturer_labels');
	}
	...
	function runPageMain()
	{
		...
		{
			//CApp::bounce('main.php?page=admin_main&day=' . date('Y-m-d', $request_date));
		}
		...
		else if (!empty($_REQUEST['session']) && is_numeric($_REQUEST['session']))
		{
			...
			{
				$requested_session = CGPC::doClean($_REQUEST['session']);
				$request_date = strtotime($session_info_array[$requested_session]['session_start']);
			}
		}
		...
		if ($this->show['store_selector'])
		{
			...
			{
				CBrowserSession::setCurrentFadminStore(CGPC::do_clean($_POST['store'],TYPE_INT));
			}
		...
		$Store->find(true);
		if (!empty($Store->hide_fadmin_home_dashboard) && (CUser::getCurrentUser()->user_type == CUser::FRANCHISE_MANAGER ||
																CUser::getCurrentUser()->user_type == CUser::FRANCHISE_OWNER ||
																CUser::getCurrentUser()->user_type == CUser::OPS_LEAD))
		{
		...
		list($dashboard_update_required, $dashboard_metrics) = CDashboardMenuBased::getMetricsSnapShot($Store->id, date('Y-m-d', $request_date));

...
Date and time: <?php $this->sessionTypeDateTimeFormat($this->sessionInfo['session_start'], $this->sessionInfo['session_type_subtype'], VERBOSE)?>
...
Rate your meals now > <?php echo HTTPS_BASE ?>my-meals
<?php } ?>

---------------------------
Not feeling well?
If you are experiencing a fever or other illness symptoms within 24 hours of your pick up or assembly session, please call <?php echo $this->sessionInfo['telephone_day'] . "\n";?> to reschedule your visit.
...
Misc Food ( <?php echo $this->orderInfo['misc_food_subtotal_desc']?> ): $<?php echo $this->moneyFormat($this->orderInfo['misc_food_subtotal']) ?><?php } ?>
 <?php if( !$this->isEmptyFloat($this->orderInfo['misc_nonfood_subtotal']) ) { ?>
Misc Non-Food ( <?php echo $this->orderInfo['misc_nonfood_subtotal_desc']?> ): $<?php echo $this->moneyFormat($this->orderInfo['misc_nonfood_subtotal']) ?><?php } ?>
...
Volume Reward:		-<?php echo $this->moneyFormat($this->orderInfo['volume_discount_total']) ?><?php } ?>
<?php if( !$this->isEmptyFloat( $this->orderInfo['points_discount_total'] ) ) { ?>
PlatePoints Dinner Dollars: -<?php echo $this->moneyFormat($this->orderInfo['points_discount_total']) ?><?php } ?>
<?php if( !$this->isEmptyFloat( $this->orderInfo['dream_rewards_discount'] ) ) { ?>
Dream Rewards Discount: -<?php echo $this->moneyFormat( $this->orderInfo['dream_rewards_discount'] ) ?><?php } ?>
<?php if( !$this->isEmptyFloat( $this->orderInfo['promo_code_discount_total'] ) ) { ?>
Promotional Code Discount: -<?php echo $this->moneyFormat( $this->orderInfo['promo_code_discount_total'] ) ?><?php } ?>
<?php if( !$this->isEmptyFloat( $this->orderInfo['coupon_code_discount_total'] ) ) { ?>
Promo Code Discount (<?php echo $this->orderInfo['coupon_title']?>): -<?php echo $this->moneyFormat( $this->orderInfo['coupon_code_discount_total'] ) ?><?php } ?>
...
Average Cost Per Serving: <?php echo $this->moneyFormat($this->orderInfo['average_per_serving_cost']) ?>
...
?>
Avg Cost Per Serving for Dinners: $<?php echo $this->moneyFormat( COrders::averageCostPerServing($this->orderInfo, false, $basisAdjustment) ) ?>
<?php } } ?>

...
- Plan to order in-store for your next session. Save time by <?=HTTPS_BASE?>?page=print&menu=<?=$this->next_menu_id?>&store=<?=$this->store_id?> taking a peek at next month's menu before you arrive and you'll be all set to order when you come in. Plus, you earn bonus PLATEPOINTS each time you sign up in-store.
...
Contact your local store: <?=HTTPS_BASE?>?page=locations&store_id=<?=$this->store_id?>
View Dream Dinners Policy, Terms and Conditions: <?=HTTPS_BASE?>?static=terms
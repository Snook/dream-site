										...
										<div class="form-group col-6 pl-4">
											...
											<?php if (!empty($this->current_session_id) && $this->current_session_id == $session['id']) { ?>
												<div class="custom-control custom-radio">
													...
													<label class="custom-control-label px-2<?php echo (!empty($this->current_session_id) && $this->current_session_id == $session['id']) ? ' bg-green font-weight-bold text-white' : ''; ?>" for="sessionRescheduleRadio-<?php echo $session['id']; ?>"><?php echo CTemplate::dateTimeFormat($session['session_start'], TIME_ONLY); ?> - <?php echo CTemplate::dateTimeFormat($session['session_end'], TIME_ONLY); ?> (current) <?php echo (false && !empty($session['session_password'])) ? '<i class="fas fa-key" data-toggle="tooltip" data-placement="top" data-tooltip="Private party, password required"></i>' : ''; ?></label>
												</div>
											<?php } else { ?>
												<div class="custom-control custom-radio">
													<input type="radio" id="sessionRescheduleRadio-<?php echo $session['id']; ?>" data-day="<?php echo $date; ?>"
													...
													<label class="custom-control-label px-2<?php echo (!empty($this->current_session_id) && $this->current_session_id == $session['id']) ? ' bg-green font-weight-bold text-white' : ''; ?>" for="sessionRescheduleRadio-<?php echo $session['id']; ?>"><?php echo CTemplate::dateTimeFormat($session['session_start'], TIME_ONLY); ?> - <?php echo CTemplate::dateTimeFormat($session['session_end'], TIME_ONLY); ?> <?php echo (false && !empty($session['session_password'])) ? '<i class="fas fa-key" data-toggle="tooltip" data-placement="top" data-tooltip="Private party, password required"></i>' : ''; ?></label>
												</div>
											<?php } ?>
										</div>
									<?php } ?>

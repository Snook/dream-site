		...
		<?php if (!empty($this->cart_info['order_info']['subtotal_delivery_fee']) && $this->foodState == 'adequateFood' ) { ?>
			...
			<?php } else { ?>
				<div class="row">
					...
					<div class="col-md-6 col-4 text-right">
						<p><?php if ($this->cart_info["menuObj"]->isEnabled_DeliveryDiscount($this->cart_info["storeObj"])) {?><span class="text-decoration-line-through">($19.99)</span><?php } ?> $<span id="checkout_total-delivery_fee"><?php echo CTemplate::moneyFormat($this->cart_info['order_info']['subtotal_delivery_fee']);?></span></p>
					</div>
				</div>
			<?php } ?>
		<?php } ?>

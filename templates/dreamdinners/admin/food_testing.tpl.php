...
<tr>
	<td colspan="8" class="bgcolor_dark catagory_row">
		<span style="float:right;"><input id="download_reports" type="button" value="Reports" class="button" /></span>
		<span style="float:right;"><input id="create_survey" type="button" value="Add Recipes" class="button" /></span>
		Food Testing
	</td>
</tr>
</thead>
...
<tr>
	<td class="bgcolor_lighter" id="recipe_row-<?php echo $id; ?>"><div id="recipe_row_disc-<?php echo $id; ?>" class="disc_closed"></div><a href="main.php?page=admin_food_testing&amp;recipe=<?php echo $id; ?>"><?php echo $recipe['title']; ?></a></td>
	...
	<td class="bgcolor_lighter" style="text-align:center;">
		...
		<?php if ($recipe['file_name']) { ?><input type="button" id="download_files-<?php echo $id; ?>" data-file_id="<?php echo $recipe['file_id']; ?>" name="download_files-<?php echo $id; ?>" value="V" data-tooltip="<?php echo $recipe['file_name']; ?>" class="button" style="margin-left:-7px;width: 20px;" /><?php } ?>
	</td>
	<td class="bgcolor_lighter" style="text-align:center;">
		<input type="button" id="export_store_report-<?php echo $id; ?>" name="export_store_report-<?php echo $id; ?>" value="Store" class="button" <?php if (empty($recipe['total_stores'])) { ?>disabled="disabled"<?php } ?> />
		<input type="button" id="export_guest_report-<?php echo $id; ?>" name="export_guest_report-<?php echo $id; ?>" value="Guest" class="button" <?php if (empty($recipe['total_stores'])) { ?>disabled="disabled"<?php } ?> />
	</td>
...
<tr>
	<td colspan="8">
		...
		<tr>
			<td class="bgcolor_light"><a href="main.php?page=admin_food_testing_survey&amp;recipe=<?php echo $survey['id']; ?>&amp;store_id=<?php echo $survey['store_id']; ?>"><?php echo $survey['store_name']; ?></a></td>
			...
			<td class="bgcolor_light" style="text-align:center;" id="store_paid_td-<?php echo $survey['id']; ?>">
				<div  id="store_paid_div-<?php echo $survey['store_id']; ?>">
					...
					<?php } else { ?>
						<input type="button" id="mark_paid-<?php echo $survey['id']; ?>" name="mark_paid-<?php echo $survey['id']; ?>" data-store_paid_id="<?php echo $survey['store_id']; ?>" value="Mark Paid" class="button" <?php if (empty($survey['food_testing_w9'])) { ?>style="display:none;"<?php } ?> />
					<?php } ?>
				</div>
				<?php if (empty($survey['food_testing_w9'])) { ?>
				<div id="store_w9_div-<?php echo $survey['store_id']; ?>">
					<input type="button" id="received_w9-<?php echo $survey['id']; ?>" name="received_w9-<?php echo $survey['id']; ?>" data-store_w9_id="<?php echo $survey['store_id']; ?>" value="W9 Received" class="button" />
				</div>
				<?php } ?>
			</td>
		</tr>
...
<br /><input type="submit" value="Add Recipes" class="button" />
</form>
</div>
...
<input type="file" name="add_survey_file" id="add_survey_file"><br /><br /><input type="submit" value="Add File" class="button" />
</form>
</div>
...
<br /><input type="submit" value="Add Stores" class="button" />
</form>
</div>

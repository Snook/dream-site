...
if ($this->success == true && $this->interface == 0)
{
	...
	foreach ($this->order_details as $id => $entity)
	{
		...
		if ($isFinishingTouch)
		{
			$pdf->Four_Up_Add_Finishing_Touch_PDF_Label($txt1, $txt2, $txt3, $txt4, $entity['prep_time'], $showBorders, $overrideLineHeight, 0, $entity['store_name'], $entity['store_phone'], $entity);
		}
		else
		{
			$pdf->Four_Up_Add_PDF_Label($entity, $title, $instructions, $servingType, false, $showBorders, $overrideLineHeight);
					...
					<tr>
						...
						<td>
							...
							<?php if (isset($this->form_list['menu_items_html'])) { ?>
								<div style="float:right"><input type="submit" class="button" value="Generate All" name="report_all" /> This will generate 1 page for every
					...
					<tr>
						...
						<td>
							...
							<?php if (isset($this->form_list['ft_menu_items_html'])) { ?>
								<div style="float:right"><input type="submit" class="button" value="Generate All" name="ft_report_all" /> This will generate 1 page for every

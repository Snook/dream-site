		...
		<table class="ME_menu_editor_table" style="width:100%;">
			...
			<tr>
				...
				<td align="left" style="padding-left: 10px;">
					<a href="/backoffice/menu-editor" class="btn btn-primary btn-sm">Menu Editor</a>
				</td>
			</tr>
		</table>
		<div class="row" >
										...
										if (is_array($planNode) && $planNode['pricing_type'] == CMenuItem::FULL)
										{
											...
											<tr id="row_<?= $planNode['id'] ?>" data-menu_item_id="<?= $planNode['id'] ?>" data-ltd_menu_item_value="<?= $planNode['ltd_menu_item_value'] ?>">
												...
												<td nowrap align="left" class="ME_zebra_even" style="padding: 2px;">
													...
													<a href="/item?recipe=<?php echo $planNode['recipe_id']; ?>&amp;ov_menu=<?php echo $this->menuInfo['menu_id'];?>" data-tooltip="Item Page" target="_blank">&gt;</a>
											...
											<tr id="tr_act_<?php echo $planNode['recipe_id']; ?>" class="collapse">
												...
												<td colspan="3" id="td_act_poi_<?php echo $planNode['recipe_id']; ?>" nowrap class="actuals_cell thick_left_border" style="padding:4px;">
													...
													else
													{
														...
														if (!empty($this->sales_summary[$planNode['recipe_id']]['TOTAL'][CMenuItem::HALF]))
														{
															$total += (int)$this->sales_summary[$planNode['recipe_id']]['TOTAL'][CMenuItem::HALF];
														}

														if (!empty($this->sales_summary[$planNode['recipe_id']]['TOTAL'][CMenuItem::FULL]))
														{
															$total += (int)$this->sales_summary[$planNode['recipe_id']]['TOTAL'][CMenuItem::FULL];
														}

														$actualStr = "<b>" . $total . " Svg</b> (";

														if (!empty($this->sales_summary[$planNode['recipe_id']]['TOTAL'][CMenuItem::HALF]))
														{
															$actualStr .= "M:" . (int)$this->sales_summary[$planNode['recipe_id']]['TOTAL'][CMenuItem::HALF] / 3;
														}

														if (!empty($this->sales_summary[$planNode['recipe_id']]['TOTAL'][CMenuItem::FULL]))
														{
															$actualStr .= "L:" . (int)$this->sales_summary[$planNode['recipe_id']]['TOTAL'][CMenuItem::FULL] / 6;
														}
								...
								{ ?>
									<tr id="row_<?= $ctsItem['id'] ?>" data-menu_item_id="<?= $ctsItem['id'] ?>" data-ltd_menu_item_value="<?= $ctsItem['ltd_menu_item_value'] ?>">
										...
										<td nowrap align="left" class="ME_zebra_even" style="padding: 2px;">
											...
											<a href="/item?recipe=<?php echo $ctsItem['recipe_id']; ?>&amp;ov_menu=<?php echo $this->menuInfo['menu_id'];?>" data-tooltip="Item Page" target="_blank">&gt;</a>

<table id="recipe_list_yes" style="width: 100%;">
	<thead>
	<tr>
		<th colspan="4" style="text-align: right;"><input id="filter" name="filter" type="text" placeholder="Filter" /> <span id="clear_filter" class="button">Clear</span></th>
	</tr>
	</thead>
	<tbody>
	</tbody>
		...
		<?php foreach ($this->menuItems as $menu_id => $menuItems) { ?>
			...
			<?php foreach ($menuItems['items'] as $recipe_id => $menuItem) { ?>
				<tr class="bgcolor_light" data-recipe_id_row="<?php echo $recipe_id; ?>">
					<td style="width:50px;"><button class="button" data-info_menu_side="<?php echo $menuItem['id']; ?>" data-recipe_id="<?php echo $recipe_id; ?>" data-entree_id="<?php echo $menuItem['id']; ?>">Info</button></td>
					...
					<td style="text-align: right; width: 90px;">
						<span data-add_menu_side="<?php echo $menuItem['id']; ?>" data-entree_id="<?php echo $menuItem['id']; ?>" data-recipe_id="<?php echo $recipe_id; ?>" class="button">Add</span>
						<span style="display:none;" data-rmv_menu_side="<?php echo $menuItem['id']; ?>" data-entree_id="<?php echo $menuItem['id']; ?>" data-recipe_id="<?php echo $recipe_id; ?>" class="button">Cancel</span>
					</td>
				</tr>
			<?php } ?>
		<?php } ?>

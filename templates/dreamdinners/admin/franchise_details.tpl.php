...
<div class="tabbed-content" data-tabid="store">
	...
	<div class="tabs-container">
		...
		<?php } ?>
		<?php } ?>
		<?php if ($this->permission['can_add_store']) { ?><li data-tabid="add_store" data-tooltip="Add New Store" class="tab" data-link="main.php?page=admin_create_store&franchise_id=<?php echo $this->franchise['id']; ?>&back=<?php echo urlencode($_SERVER['REQUEST_URI']); ?>">+</li><?php } ?>
		</ul>
	</div>
			...
			<tr>
				<td colspan="3">
					...
					<tr>
						...
						<td class="bgcolor_light" style="text-align:center;"><a href="main.php?page=store&amp;id=<?php echo $store['id']; ?>"><?php echo (!empty($userInfo['display_to_public'])) ? 'Yes' : 'No'; ?></a></td>
					</tr>
					<?php } } ?>
					</table>
				</td>

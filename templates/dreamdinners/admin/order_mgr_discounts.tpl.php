					...
					<div class="dd_accordion_container">
						...
						<div class="dd_accordion_content" style="display:none">
							<ul>
									...
									foreach ($this->dd_history as $entry)
									{
										...
										switch ($entry['state'])
										{
											case 'AVAILABLE':
												...
												break;
											case 'CONSUMED':
												echo '$' . $entry['amount'] . ' applied to <a href="/backoffice/order-mgr?order=' . $entry['orders'] . '" target="_blank">' . $orderStr . '</a>. ';
												echo $expiredStr;
												break;
											case 'EXPIRED':
								...
								<li style="list-style-type: none !important;"><a href="/backoffice/user_plate_points?id=<?php echo $this->user['id']; ?>&amp;back=<?php echo urlencode($_SERVER['REQUEST_URI']);?>" target="_blank">Full Dinner Dollar/PlatePoint History</a></li>
							</ul>
						</div>
					</div>
					<?php } ?>

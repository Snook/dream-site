...
<?php } else { ?>
	<input type="button" class="button" value="Back" onClick="window.location = 'main.php?page=admin_user_details&id=<?= $this->customer_id?>';">
<?php } ?>
...
<h2>PLATEPOINTS Dinner Dollars</h2>
...
<tr>
	<td class="bgcolor_dark catagory_row" colspan="5" style="text-align:center;">PLATEPOINTS Dinner Dollars History</td>
</tr>
<tr id="avail_credits_header_row">
	<td class="bgcolor_medium header_row">Dinner Dollar State</td>
	<td class="bgcolor_medium header_row">Date Dinner Dollars Awarded</td>
...
<?php if (isset($this->PP_Credits) && !empty($this->PP_Credits)) {
	foreach ($this->PP_Credits as $id => $row) {
...
?>
<tr>
	<td class="<?php echo $css;?>"><?= $row['state']?></td>
	<td class="<?php echo $css;?>"><?php echo CTemplate::dateTimeFormat($row['timestamp'], NORMAL, $this->store_id, CONCISE);?></td>
	<td class="<?php echo $css;?>"><?php echo ucwords(strtolower(str_replace('_', ' ', str_replace(',', ', ', $row['events'])))); ?></td>
	<td class="<?php echo $css;?>"><?= CTemplate::dateTimeFormat($row['expires'], NORMAL, $this->store_id, CONCISE);?></td>
	<td class="<?php echo $css;?>"><?= $row['amount']?></td>
</tr>
<?php } } else { ?>
	<tr>
	  <td class="bgcolor_light" colspan="5" style="text-align:center;"><i>There are no PLATEPOINTS Dinner Dollars for this customer.</i></td></tr>

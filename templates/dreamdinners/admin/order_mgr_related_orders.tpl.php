	...
	<tr>
		<td class="order_history_cell" colspan="8" style="text-align: right;">
			...
			<?php if ($this->current_order != $order['order_id'] ) { ?>
				<a class="button" href="/backoffice/order-history?id=<?php echo $order['user_id']; ?>&amp;order=<?php echo $order['order_id']; ?>&amp;back=<?php echo urlencode($_SERVER['REQUEST_URI']); ?>">View Order</a>
				<?php if ($order['canEdit']) { ?><a class="button" href="/backoffice/order-mgr?order=<?=$order['order_id'];?>&back=<?=urlencode($_SERVER['REQUEST_URI'])?>">Edit</a><?php } ?>
			<?php } ?>
			...
			<?php if ($order['session_start'] > '2014-11-01 00:00:00' && $order['status'] != CBooking::CANCELLED) { ?>
				<a href="/print?order=<?php echo $order['order_id']; ?>&amp;freezer=true" class="button" target="_blank">Freezer Sheet</a>
...
<table width="100%" border="0">
	...
	<tr>
		<td class="order_history_cell" colspan="7" style="text-align: right;">
			...
			<?php if ($this->current_order != $order['order_id'] ) { ?>
				<a class="button" href="/backoffice/order-history?id=<?php echo $order['user_id']; ?>&amp;order=<?php echo $order['order_id']; ?>&amp;back=<?php echo urlencode($_SERVER['REQUEST_URI']); ?>">View Order</a>
				<?php if ($order['can_edit']) { ?><a class="button" href="/backoffice/order-mgr?order=<?=$order['order_id'];?>&back=<?=urlencode($_SERVER['REQUEST_URI'])?>">Edit</a><?php } ?>
			<?php } ?>

...
class DB_DataObject extends DB_DataObject_Overload
{
	...
	function pid()
	{
		...
		if (empty($this->$k))
		{ // we do not
			$this->raiseError("pid() called on Object where primary key value not available", DB_DATAOBJECT_ERROR_NODATA);
	...
	function _build_select()
	{
		...
		if (!empty($this->_query['derive_table']) && !empty($this->_query['derive_select']))
		{
			...
			$sql = 'SELECT ' . $this->_query['derive_select'] . ' FROM ( SELECT' . $this->_query['data_select'] . " \n" . " FROM   $tn \n" . $this->_join . " \n" . $this->_query['condition'] . " \n" . $this->_query['group_by'] . " \n" . $this->_query['having'] . " \n" . ') ' . $this->_query['derive_table'];

			return $sql;
		}

		$sql = 'SELECT ' . $this->_query['data_select'] . " \n" . " FROM   $tn \n" . $this->_join . " \n" . $this->_query['condition'] . " \n" . $this->_query['group_by'] . " \n" . $this->_query['having'] . " \n";
	...
	function fetch()
	{
		...
		if (empty($_DB_DATAOBJECT['RESULTS'][$this->_DB_resultid]) || !is_object($result = $_DB_DATAOBJECT['RESULTS'][$this->_DB_resultid]))
		{
			if (!empty($_DB_DATAOBJECT['CONFIG']['debug']))
			{
		...
		if ($array === null)
		{
			if (!empty($_DB_DATAOBJECT['CONFIG']['debug']))
			{
				...
				$this->debug("Last Data Fetch'ed after " . ($t[0] + $t[1] - $_DB_DATAOBJECT['QUERYENDTIME']) . " seconds", "FETCH", 1);
			}
		...
		foreach ($array as $k => $v)
		{
			...
			$kk = (strpos($k, '.') === false && strpos($k, ' ') === false && strpos($k, '@') === false) ? $k : str_replace($replace, '_', $k);
	...
	function fetchAll($k = false, $v = false, $method = false)
	{
		...
		if ($k !== false && (   // only do this is we have not been explicit..
				empty($this->_query['data_select']) || ($this->_query['data_select'] == '*')))
		{
			$this->selectAdd();
			$this->selectAdd($k);
			if ($v !== false)
	...
	function whereAdd($cond = false, $logic = 'AND')
	{
		...
		{
			return $this->raiseError("You cannot do two queries on the same object (clone it before finding)", DB_DATAOBJECT_ERROR_INVALIDARGS);
		}
	...
	function limit($a = null, $b = null)
	{
		...
		// check input...= 0 or '    ' == error!
		if ((!is_int($a) && ((string)((int)$a) !== (string)$a)) || (($b !== null) && (!is_int($b) && ((string)((int)$b) !== (string)$b))))
		{
	...
	function insert($ignoreAffectedRows = false)
	{
		...
		foreach ($items as $k => $v)
		{
			...
			{
				$rightq .= $this->_quote((string)(($v & DB_DATAOBJECT_BOOL) ? // this is thanks to the braindead idea of postgres to
						// use t/f for boolean.
	...
	function update($dataObject = false)
	{
		...
		$this->raiseError("update: No Data specifed for query $settings , {$this->_query['condition']}", DB_DATAOBJECT_ERROR_NODATA);
	...
	function delete($useWhere = false)
	{
		...
		if (($this->_query !== false) && $this->_query['condition'])
		{
			...
			$sql .= (!empty($this->_join) && $useWhere) ? "{$table} FROM {$table} {$this->_join} " : "FROM {$table} ";
	...
	function fetchRow($row = null)
	{
		...
		{
			// use strpos as str_replace is slow.
			$kk = (strpos($k, '.') === false && strpos($k, ' ') === false) ? $k : str_replace($replace, '_', $k);
	...
	function count($countWhat = false, $whereAddOnly = false)
	{
		...
		if (!isset($t->_query))
		{
			$this->raiseError("You cannot do run count after you have run fetch()", DB_DATAOBJECT_ERROR_INVALIDARGS);
		...
		{
			$this->raiseError("You cannot do run count without keys - use \$do->count('id'), or use \$do->count('distinct id')';", DB_DATAOBJECT_ERROR_INVALIDARGS, PEAR_ERROR_DIE);
		...
		$countWhat = (strtoupper($countWhat) == 'DISTINCT') ? "DISTINCT {$table}.{$key_col}" : $countWhat;

		$countWhat = is_string($countWhat) ? $countWhat : "{$table}.{$key_col}";

		$r = $t->_query("SELECT count({$countWhat}) as $as
	...
	var $_query = array(
		...
		// the added unions,
		'derive_table' => '',
		// derived table name (BETA)
		'derive_select' => '',
		// derived table select (BETA)
	);
	...
	function sequenceKey()
	{
		...
		if (!$keys)
		{
			return $_DB_DATAOBJECT['SEQUENCE'][$this->_database][$this->tableName()] = array(
				false,
				false,
				false
			);
		...
		if (in_array($dbtype, array('pgsql')) && ($table[$usekey] & DB_DATAOBJECT_INT) && isset($realkeys[$usekey]) && strlen($realkeys[$usekey]) > 1)
		{
			return $_DB_DATAOBJECT['SEQUENCE'][$this->_database][$this->tableName()] = array(
				$usekey,
				true,
		...
		if (in_array($dbtype, array(
				...
				'mssql',
				'ifx'
			)) && ($table[$usekey] & DB_DATAOBJECT_INT) && isset($realkeys[$usekey]) && ($realkeys[$usekey] == 'N'))
		{
			return $_DB_DATAOBJECT['SEQUENCE'][$this->_database][$this->tableName()] = array(
				$usekey,
				true,
		...
		if (($realkeys[$usekey] != 'N') && !empty($_DB_DATAOBJECT['CONFIG']['dont_use_pear_sequences']))
		{
			return array(
				false,
				false,
	...
	function _quote($str)
	{
		...
		return (empty($_DB_DATAOBJECT['CONFIG']['db_driver']) || ($_DB_DATAOBJECT['CONFIG']['db_driver'] == 'DB')) ? $_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]->quoteSmart($str) : $_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]->quote($str);
	}
	...
	function _connect()
	{
		...
		if ($this->_database_dsn_md5 && !empty($_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]))
		{
			...
			{
				return $this->raiseError($_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]->message, $_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]->code, PEAR_ERROR_DIE);
			}
			...
			if (empty($this->_database))
			{
				...
				$this->_database = ($db_driver != 'DB' && $hasGetDatabase) ? $_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]->getDatabase() : $_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]->dsn['database'];

				if (($_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]->dsn['phptype'] == 'sqlite') && is_file($this->_database))
				{
		...
		if (!$dsn)
		{
			return $this->raiseError("No database name / dsn found anywhere", DB_DATAOBJECT_ERROR_INVALIDCONFIG, PEAR_ERROR_DIE);
		}
		...
		else
		{
			...
			$db_options['portability'] = isset($db_options['portability']) ? $db_options['portability'] : MDB2_PORTABILITY_ALL ^ MDB2_PORTABILITY_FIX_CASE;
		...
		if (PEAR::isError($_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]))
		{
			...
			return $this->raiseError("Connect failed, turn on debugging to 5 see why", $_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]->code, PEAR_ERROR_DIE);
		}
	...
	function _query($string, $ignoreAffectedRows = false)
	{
		...
		if (strtoupper($string) == 'BEGIN' || strtoupper($string) == 'START TRANSACTION')
		{
			if ($_DB_driver == 'DB')
			{
				$DB->autoCommit(false);
	...
	function _build_condition($keys, $filter = array(), $negative_filter = array())
	{
		...
		foreach ($keys as $k => $v)
		{
			...
			$kSql = $quoteIdentifiers ? ($DB->quoteIdentifier($this->tableName()) . '.' . $DB->quoteIdentifier($k)) : "{$this->tableName()}.{$k}";
			...
			if ($v & DB_DATAOBJECT_STR)
			{
				$this->whereAdd(" $kSql  = " . $this->_quote((string)(($v & DB_DATAOBJECT_BOOL) ? // this is thanks to the braindead idea of postgres to
						// use t/f for boolean.
						(($this->$k === 'f') ? 0 : (int)(bool)$this->$k) : $this->$k)));
				continue;
			}
	...
	function factory($table = '')
	{
		...
		if ($table === '')
		{
			...
			{
				return DB_DataObject::raiseError("factory did not receive a table name", DB_DATAOBJECT_ERROR_INVALIDARGS);
			}
		}
	...
	function links()
	{
		...
		$schemas = isset($cfg['schema_location']) ? array("{$cfg['schema_location']}/{$this->_database}.ini") : array();

		// if ini_* is set look there instead.
		// and support multiple locations.
		if (isset($cfg["ini_{$this->_database}"]))
		{
			$schemas = is_array($cfg["ini_{$this->_database}"]) ? $cfg["ini_{$this->_database}"] : explode(PATH_SEPARATOR, $cfg["ini_{$this->_database}"]);
		}
		...
		foreach ($schemas as $ini)
		{
			...
			$links = isset($cfg["links_{$this->_database}"]) ? $cfg["links_{$this->_database}"] : str_replace('.ini', '.links.ini', $ini);
			...
			$lcfg[$this->_database] = array_merge($lcfg[$this->_database], parse_ini_file($links, true));
		...
		if (!empty($_DB_DATAOBJECT['CONFIG']['portability']) && $_DB_DATAOBJECT['CONFIG']['portability'] & 1)
		{
			foreach ($lcfg[$this->_database] as $k => $v)
			{
				...
				$nk = strtolower($k);
				// results in duplicate cols.. but not a big issue..
				$lcfg[$this->_database][$nk] = isset($lcfg[$this->_database][$nk]) ? $lcfg[$this->_database][$nk] : array();
	...
	function joinAdd($obj = false, $joinType = 'INNER', $joinAs = false, $joinCol = false)
	{
		...
		if (is_array($joinType))
		{
			...
			$useLinks = isset($joinType['useLinks']) ? $joinType['useLinks'] : true;
		...
		$objTable = $quoteIdentifiers ? $DB->quoteIdentifier($obj->tableName()) : $obj->tableName();

		$dbPrefix = '';
		if (strlen($obj->_database) && in_array($DB->dsn['phptype'], array(
				'mysql',
				'mysqli'
			)))
		{
		...
		if (strtolower(get_class($obj)) != 'db_dataobject')
		{
			...
			foreach ($items as $k => $v)
			{
				...
				if ($v & DB_DATAOBJECT_STR)
				{
					$obj->whereAdd("{$joinAs}.{$kSql} = " . $this->_quote((string)(($v & DB_DATAOBJECT_BOOL) ? // this is thanks to the braindead idea of postgres to
							// use t/f for boolean.
							(($obj->$k === 'f') ? 0 : (int)(bool)$obj->$k) : $obj->$k)));
					continue;
				}
				if (is_numeric($obj->$k))
				{
		...
		if ($quoteIdentifiers)
		{
			...
			$ofield = (is_array($ofield)) ? array_map(array(
				$DB,
				'quoteIdentifier'
			), $ofield) : $DB->quoteIdentifier($ofield);
			$tfield = (is_array($tfield)) ? array_map(array(
				$DB,
				'quoteIdentifier'
		...
		if ($addJoinAs)
		{
			...
			// for those databases.
			$fullJoinAs = in_array($DB->dsn["phptype"], array(
				'mysql',
				'mysqli',
				'pgsql'
			)) ? "AS {$joinAs}" : $joinAs;
	...
	function autoJoin($cfg = array())
	{
		...
		if (!empty($cfg['distinct']) && $keys)
		{
			...
			$selectAs = empty($cols) ? array() : array(
				array(
					array($cols),
					'%s',
					false
	...
	function setFrom($from, $format = '%s', $skipEmpty = false)
	{
		...
		if (!$items)
		{
			$this->raiseError("setFrom:Could not find table definition for {$this->tableName()}", DB_DATAOBJECT_ERROR_INVALIDCONFIG);
		...
		foreach (array_keys($items) as $k)
		{
			...
			$chk = is_object($from) && (version_compare(phpversion(), "5.1.0", ">=") ? property_exists($from, sprintf($format, $k)) :  // php5.1
	...
	function toArray($format = '%s', $hideEmpty = false)
	{
		...
		$ret = array();
		$rf = ($this->_resultFields !== false) ? $this->_resultFields : (isset($_DB_DATAOBJECT['RESULTFIELDS'][$this->_DB_resultid]) ? $_DB_DATAOBJECT['RESULTFIELDS'][$this->_DB_resultid] : false);

		if (!empty($rf) && is_array($rf))
		{
			foreach ($rf as $key => $value)
			{
				if (strpos($key, 'join__') === 0)
				{
					unset($rf[$key]);
				}
			}
		...
		$ar = ($rf !== false) ? (($hideEmpty === 0) ? $rf : array_merge($rf, $this->table())) : $this->table();
		...
		foreach ($ar as $k => $v)
		{
			...
			if (method_exists($this, 'get' . $k) && !in_array(strtolower($k), array(
					'links',
					'link'
				)))
			{
	...
	function raiseError($message, $type = null, $behaviour = null)
	{
		...
		else
		{
			...
			$error = $p->raiseError($message, $type, $behaviour, $opts = null, $userinfo = null, 'DB_DataObject_Error');
		}
	...
	function free()
	{
		...
		if (isset($this->_DB_resultid) && isset($_DB_DATAOBJECT['RESULTFIELDS'][$this->_DB_resultid]))
		{
			unset($_DB_DATAOBJECT['RESULTFIELDS'][$this->_DB_resultid]);
		}
		if (isset($this->_DB_resultid) && isset($_DB_DATAOBJECT['RESULTS'][$this->_DB_resultid]))
		{
			unset($_DB_DATAOBJECT['RESULTS'][$this->_DB_resultid]);
		}
		...
		if (isset($this->_database_dsn_md5) && isset($_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]))
		{
			$_DB_DATAOBJECT['CONNECTIONS'][$this->_database_dsn_md5]->num_rows = array();
		}

		if (isset($this->_database_dsn_md5) && is_array($this->_link_loaded))
		{
	...
	function _is_null($obj_or_ar, $prop)
	{
		...
		$isset = $prop === false ? isset($obj_or_ar) : (is_array($obj_or_ar) ? isset($obj_or_ar[$prop]) : isset($obj_or_ar->$prop));

		$value = $isset ? ($prop === false ? $obj_or_ar : (is_array($obj_or_ar) ? $obj_or_ar[$prop] : $obj_or_ar->$prop)) : null;

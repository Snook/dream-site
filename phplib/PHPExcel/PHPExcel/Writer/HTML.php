...
class PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter {
	 ...
	 * @throws Exception
	 */
	public function generateSheetData($HyperlinkToNewTab = false) {
		// PHPExcel object known?
		if (is_null($this->_phpExcel)) {
			throw new Exception('Internal PHPExcel object not set to an instance of an object.');
		}
		...
		foreach ($sheets as $sheet) {
			...
			while($row++ < $rowMax) {
				...
				if ( !isset($this->_isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row]) ) {
					...
					$html .= $this->_generateRow($sheet, $rowData, $row - 1, $HyperlinkToNewTab);
				}
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		foreach ($sheets as $sheet) {
			...
			while($column++ < $highestColumnIndex) {
				...
				$css['table.sheet' . $sheetIndex . ' col.col' . $column]['width'] = '42px';
				$css['table.sheet' . $sheetIndex . ' col.col' . $column]['min-width'] = '42pt';
			}
			...
			foreach ($sheet->getColumnDimensions() as $columnDimension) {
				if (($width = PHPExcel_Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
					...
					$css['table.sheet' . $sheetIndex . ' col.col' . $column]['width'] = $width . 'px';
					$css['table.sheet' . $sheetIndex . ' col.col' . $column]['min-width'] = $width . 'pt';
					
					$css['table.sheet' . $sheetIndex . ' .column' . $column]['width'] = $width . 'pt';
					$css['table.sheet' . $sheetIndex . ' .column' . $column]['min-width'] = $width . 'pt';
			...
			$css['table.sheet' . $sheetIndex . ' tr']['height'] = $pt_height . 'px';
			...
			foreach ($sheet->getRowDimensions() as $rowDimension) {
				...
				$css['table.sheet' . $sheetIndex . ' tr.row' . $row]['height'] = $pt_height . 'px';
				if ($rowDimension->getVisible() === false) {
	 ...
	 * @throws	Exception
	 */
	private function _generateRow(PHPExcel_Worksheet $pSheet, $pValues = null, $pRow = 0, $HyperlinkToNewTab = false) {
		if (is_array($pValues)) {
			// Construct HTML
			$html = '';
			...
			foreach ($pValues as $cell) {
				...
				if ($pSheet->hyperlinkExists($coordinate) && !$pSheet->getHyperlink($coordinate)->isInternal()) {
				    ...
				    if ($HyperlinkToNewTab)
				    {
					   $cellData = '<a target="_blank" href="' . htmlspecialchars($pSheet->getHyperlink($coordinate)->getUrl()) . '" title="' . htmlspecialchars($pSheet->getHyperlink($coordinate)->getTooltip()) . '">' . $cellData . '</a>';
				    }
				    else 
				    {
				...
				if ($writeCell) {
					...
					$html .= '			<td';
						if (!$this->_useInlineCss) {
							...
							$cssClass['width'] = $width . 'px';

							// We must also explicitly write the height of the <td> element because TCPDF
							// does not recognize e.g. <tr style="height:50px">

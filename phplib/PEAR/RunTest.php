    ...
    function run($file, $ini_settings = '')
    {
        ...
        if (array_key_exists('SKIPIF', $section_text)) {
            if (trim($section_text['SKIPIF'])) {
                ...
                unlink($tmp_skipif);
                if (preg_match("/^skip/i", trim($output))) {
                    $skipreason = "SKIP $tested";
                    $reason = (preg_match("/^skip[[:space:]]*(.+)\$/i", trim($output))) ? eregi_replace("^skip[[:space:]]*(.+)\$", "\\1", trim($output)) : FALSE;
                    if ($reason) {

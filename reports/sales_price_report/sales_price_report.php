...
try
{
	...
	foreach ($months as $subMonths)
	{
		...
		$csv_path = "../output/sales_price_report/sales-price-report-" . $subMonths[0] . "_" . date("Y-m-d") . ".xlsx";
		...
		foreach ($activeMenus as $menu)
		{
			...
			while ($DAO_store->fetch())
			{
				...
				$arr = COrders::buildNewPricingMenuPlanArrays($DAO_store, $menu['id'], 'FeaturedFirst', true, true);
				if (!array_key_exists('Specials', $arr))
				{
					continue;
				}
				foreach ($arr['Specials'] as $key => $coreItem)
				{
		...
		$fileName = "Core Sales Price Report - Active Menus";
		$titleRows = false;
		$sectionHeader = false;
		$callbacks = false;
